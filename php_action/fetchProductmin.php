...
$sql = "SELECT produits.*, product.ratea,product.quantity,product.payment_type,product.payment_mode,product.status,fournisseurs.four_name, product.product_id, product.active FROM produits, product, fournisseurs WHERE produits.id=product.produit_id AND product.status = 1 AND fournisseurs.four_id=product.four_id AND product.quantity BETWEEN 1 AND 3 ORDER BY product.product_id DESC";
